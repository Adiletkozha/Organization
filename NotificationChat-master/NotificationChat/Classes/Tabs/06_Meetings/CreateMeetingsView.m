...
@implementation CreateMeetingsView
...
- (void)actionDone
{
    ...
    if ([name length] == 0)		{ [ProgressHUD showError:@"Meetings description must be set."]; return; }
	if ([selection count] == 0) { [ProgressHUD showError:@"Please select some users."]; return; }

    PFUser *user = [PFUser currentUser];
	[selection addObject:user.objectId];

    PFObject *object = [PFObject objectWithClassName:PF_MEETINGS_CLASS_NAME];
	object[PF_MEETINGS_USER] = [PFUser currentUser];
	object[PF_MEETINGS_MSG_MEETING] = name;
	object[PF_MEETINGS_WORKERS] = selection;
   [object setObject:_datePicker.date forKey:@"time"];
