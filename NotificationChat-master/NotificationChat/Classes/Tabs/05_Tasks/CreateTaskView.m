...
@synthesize viewHeader, fieldName, taskDescription;
...
- (void)actionDone
{
    ...
    NSString *taskDesc = taskDescription.text;

    if ([name length] == 0)		{ [ProgressHUD showError:@"Task name must be set."]; return; }
	if ([selection count] == 0) { [ProgressHUD showError:@"Please select some users."]; return; }
	
    PFUser *user = [PFUser currentUser];
	[selection addObject:user.objectId];
	
    PFObject *object = [PFObject objectWithClassName:PF_TASK_CLASS_NAME];
	object[PF_TASK_USER] = [PFUser currentUser];
	object[PF_TASK_NAME] = name;
    object[PF_TASK_DESCRIPTION] = taskDesc;
	object[PF_TASK_WORKERS] = selection;
    
    
 
    
    
    [object setObject:_datePicker.date forKey:@"deadline"];
